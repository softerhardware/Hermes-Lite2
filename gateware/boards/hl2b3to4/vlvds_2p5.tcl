
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_link_tx[*]
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_link_rx[*]
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_cn4_6
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_cn4_7
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_phone_tip
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_phone_ring
set_instance_assignment -name IO_STANDARD "2.5 V" -to io_db24_1
